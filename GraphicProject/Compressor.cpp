...
void Compressor::addSequenceOfSameSymbolsByterun(std::vector<uint8_t>& destination, const uint8_t &symbol, const uint8_t &numberOfSymbols) {
	destination.push_back(numberOfSymbols);
	destination.push_back(symbol);
}
		...
		for (int x = 0; x < pixels[y].size(); ++x) {
			...
			while (current == previous && x<pixels[y].size()-1) {
				...
				if (how_many_same == type_restriction ) {
					addSequenceOfSameSymbolsByterun(compressed, previous, how_many_same+128);	//  add symbols if number is higher then 127
			...
			if (how_many_same > 0) {
				...
				addSequenceOfSameSymbolsByterun(compressed, previous, how_many_same+128);		// add same numbers
				how_many_same = 0;																		// and because it should be on minus we save it as bigger than 127
